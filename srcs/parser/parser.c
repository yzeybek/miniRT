...
/*   Updated: 2025/10/21 12:47:40 by yzeybek             ###   ########.fr    */
/*                                                                            */
/* ************************************************************************** */
...
static int	parse_line(char *line, t_scene *scene)
{
	...
	if (*line == 'A' && ft_isnone(*(line + 1)))
		...
		ids[4] += parse_plane(line + 2, scene, ids + 4);
	else if (*line == 's' && *(line + 1) == 'p' && ft_isnone(*(line + 2)))
		ids[5] += parse_sphere(line + 2, scene, ids + 4);
	else if (*line == 'c' && *(line + 1) == 'y' && ft_isnone(*(line + 2)))
		ids[6] += parse_cylinder(line + 2, scene, ids + 4);
	else if (*line == 'c' && *(line + 1) == 'n' && ft_isnone(*(line + 2)))
		ids[7] += parse_cone(line + 2, scene, ids + 4);
	else
